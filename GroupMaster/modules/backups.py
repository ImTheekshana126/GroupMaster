...
import GroupMaster.modules.sql.notes_sql as sql
from GroupMaster import dispatcher, LOGGER, OWNER_ID, SUDO_USERS, MESSAGE_DUMP
from GroupMaster.__main__ import DATA_IMPORT
...
from GroupMaster.modules.helper_funcs.msg_types import get_note_type
from GroupMaster.modules.rules import get_rules
...
def import_data(bot: Bot, update):
	...
	if msg.reply_to_message and msg.reply_to_message.document:
		...
		try:
			...
			return msg.reply_text("There is problem while importing the data! Please ask in @GroupMasterAyaGroup about why this happened.")
		# Check if backup is from self
		try:
			if str(bot.id) != str(data[str(chat.id)]['bot']):
				return msg.reply_text("Backup from another bot that is not suggested might cause the problem, documents, photos, videos, audios, records might not work as it should be. However, You can still request a feature regarding this in @GroupMasterAyaGroup !")
		except:
			pass
		# Select data source
		...
		try:
			...
			msg.reply_text("An error occurred while recovering your data. The process failed. If you experience a problem with this, please ask in @GroupMasterAyaGroup . My owner and community will be happy to help. Also, bugs report makes me even better!\nThank you!")
...
def export_data(bot: Bot, update: Update, chat_data):
	...
	f=open("GroupMasterb{}.backup".format(chat_id), "w")
	...
	try:
		...
		pass
	bot.sendDocument(current_chat_id, document=open('GroupMasterb{}.backup'.format(chat_id), 'rb'), caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `harukab` is specially made for notes.".format(chat.title, chat_id, tgl), timeout=360, reply_to_message_id=msg.message_id, parse_mode=ParseMode.MARKDOWN)
	os.remove("GroupMasterb{}.backup".format(chat_id)) # Cleaning file
...
This module is still in beta! Report bugs in @leosupportx !
"""
